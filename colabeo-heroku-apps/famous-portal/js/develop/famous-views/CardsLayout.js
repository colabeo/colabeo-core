define(function(require, exports, module) {
    ...
    /**
     * @class Allows you to make the shown renderables behave like cards.
     * @description
     * @name CardsLayout
     * @constructor
     * @example 
    ...
    function CardsLayout(options) {
        ...
        this.sync.on('start', function(data) {
            for(var i = 0; i < this._cardStates.length; i++) {
                this._cardStates[i].reset(this._cardStates[i].get());
            }
        }.bind(this));
        ...
        this.sync.on('end', function(data) {
            if(this._activeIndex < 0) return;
            if(data.v > this.options.pageSwitchSpeed && this._activeIndex > 0) this._activeIndex--;
            else if(data.v < -this.options.pageSwitchSpeed && this._activeIndex < this._cards.length - 1) this._activeIndex++;
            var i = this._activeIndex;
            var currentCard = this._cards[i];
            var finalPosition = this.getSize()[0] - _getCardSize.call(this, currentCard);
            var augmentedTransition = Object.create(this.options.transition);
            augmentedTransition.v = data.v;
            this._cardStates[i].set(finalPosition, augmentedTransition);
        }.bind(this));
    }
    ...
    function _getCurrentCardPosition() {
        ...
        return this._cardStates[this._activeIndex].get();
    }
    ...
    CardsLayout.prototype.show = function(card) {
        ...
        if(removedIndex < 0) {
            cardState = new Transitionable(0);
        ...
        if(this._cardStates.length) {
            finalPosition = this._cardStates[this._cardStates.length - 1].get() + _getCardSize(this, this._cards[this._cardStates.length - 1]);
        }
        finalPosition = Math.min(finalPosition, this.getSize()[0] - _getCardSize.call(this, card));
        cardState.set(finalPosition, this.options.transition);
    ...
    CardsLayout.prototype.remove = function(position) {
        ...
        for(var i = position; i < this._cards.length; i++) {
            this._cardStates[i].set(this.getSize()[0], this.options.transition, function(card) {
                var i = this._removedCards.indexOf(card);
                if(i < 0) return; // card not found (added back)
                this._removedCardStates.splice(i, 1);
                this._removedCards.splice(i, 1);
            }.bind(this, this._cards[i]));
        ...
        if(this._activeIndex >= 0 && this._activeIndex < position) {
            ...
            this._cardStates[this._activeIndex].set(finalPosition, this.options.transition);
        }
    };
    ...
    function _actualRender() {
        ...
        if(this._incomingIndex >= 0) {
            var incomingPos = this._cardStates[this._incomingIndex].get();
            var targetPos = containerSize;
            if(this._activeIndex >= 0) {
                targetPos = Math.min(targetPos, this._cardStates[this._activeIndex].get() + _getCardSize.call(this, this._cards[this._activeIndex]));
            }
        ...
        if(this._cardStates.length) {
            ...
            if(this._activeIndex >= 0) {
                var activePosition = this._cardStates[this._activeIndex].get();
                ...
                for(var i = this._activeIndex - 1; i >= 0; i--) {
                    ...
                    this._cardStates[i].set((currentPosition > 0) ? currentPosition : 0);
                };
            ...
            for(var i = 0; i < this._cards.length; i++) {
                var position = this._cardStates[i].get();
                result[i] = {
        ...
        for(var i = 0; i < this._removedCardStates.length; i++) {
            var position = this._removedCardStates[i].get();
            if(this._removedCards[i]) {
                result.push({
