define(function(require, exports, module) {
    ...
    /** 
     * Document events to which Surface automatically listens.
     *   Bind to them with .on(), forward them with .pipe(), and similar.
     *
     * @name Surface#surfaceEvents
     * @field
     * @type {Array.<string>} 
     */
    Surface.prototype.surfaceEvents = ['touchstart', 'touchmove', 'touchend', 'touchcancel', 'click', 'mousedown', 'mouseup', 'mousemove', 'mouseover', 'mouseout', 'mouseenter', 'mouseleave', 'mousewheel', 'wheel'];
     ...
     */ 
    if(usePrefix) _setMatrix = function(element, matrix) { element.style.webkitTransform = Matrix.formatCSS(matrix); };
    else _setMatrix = function(element, matrix) { element.style.transform = Matrix.formatCSS(matrix); };
     ...
     * @function
     */
    Surface.prototype.commit = function(context, matrix, opacity, origin, size) {
        ...
        if(!xyEquals(this._origin, origin) || !Matrix.equals(this._matrix, matrix)) {
            ...
            var aaMatrix = matrix;
            if(origin) {
                aaMatrix = Matrix.moveThen([-this._size[0]*origin[0], -this._size[1]*origin[1]], matrix);
            }
