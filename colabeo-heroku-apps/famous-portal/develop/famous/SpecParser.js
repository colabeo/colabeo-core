define(function(require, exports, module) {
    ...
    SpecParser.prototype.parse = function(spec) {
        this.reset();
        this._parseSpec(spec, Matrix.identity, 1, [0, 0], undefined, Matrix.identity);
        return this.result;
    };
    ...
    SpecParser.prototype._parseSpec = function(spec, parentTransform, parentOpacity, parentOrigin, parentSize, sizeCtx) {
        if(spec === undefined) {
            ...
            this.result[id] = [
                Matrix.move(parentTransform, _vecInContext(originAdjust, sizeCtx)),
                parentOpacity,
                parentOrigin,
                parentSize
            ...
            if(spec.transform) transform = Matrix.multiply(parentTransform, spec.transform);
            if(spec.origin) origin = spec.origin;
            if(spec.size) {
                size = spec.size;
                if(parentSize && origin) {
                    size = [spec.size[0] || parentSize[0], spec.size[1] || parentSize[1]];
                    transform = Matrix.move(transform, _vecInContext([origin[0]*parentSize[0], origin[1]*parentSize[1], 0], sizeCtx));
                    transform = Matrix.moveThen([-origin[0]*size[0], -origin[1]*size[1], 0], transform);
                }
                sizeCtx = parentTransform;
                origin = [0, 0];
            }
