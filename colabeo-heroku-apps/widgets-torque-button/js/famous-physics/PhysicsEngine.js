...
//    TODO: deprecate
    ...
    function attachOne(agent, targets, source){
        ...
        var agentID = this._currAgentId;
        var agentArray = _mapAgentArray.call(this, agent);
        agentArray.push(agentID);
    ...
    function _updateAcceleration(particle){ if (!particle.immunity) particle.updateAcceleration(); };
    function _updateVelocity(particle, dt){ if (!particle.immunity) this._integrator.integrateVelocity(particle, dt); };
    function _updateAngularVelocity(particle){ if (!particle.immunity && particle instanceof Body) particle.updateAngularVelocity(); };
    function _updateAngularMomentum(particle, dt){ if (!particle.immunity && particle instanceof Body) this._integrator.integrateAngularMomentum(particle, dt); };
    function _updatePosition(particle, dt){ if (!particle.immunity) this._integrator.integratePosition(particle, dt); };
    function _updateOrientation(particle, dt){ if (!particle.immunity && particle instanceof Body) this._integrator.integrateOrientation(particle, dt); };

    function updateAccelerations(){     this.forEachParticle(_updateAcceleration); };
...
//        };
    ...
    function _renderParticle(particle, result){ result.push(particle.render());}
    PhysicsEngine.prototype.render = function(){
        this.step();
        var result = [];
        this.forEachParticle(_renderParticle, result);
        return result;
    };
